...
// Main Current Reading
  ...
  {
    current = analogRead(MCread);     //Reads current input and records maximum and minimum current
  ...
  RMSCurrent = ((maxCurrent - 516)*0.707)/11.8337;    //Calculates RMS current based on maximum value
  RMSPower = Veff*RMSCurrent;                         //Calculates RMS Power based on voltage reading
...
// Main Voltage Reading
  for ( int i = 0; i < 100; i++ ) {
    sensorValue1 = analogRead(MVread);
    if (analogRead(MVread) > 511) {
      val[i] = sensorValue1;
    }
    else {
      val[i] = 0;
...
// Main Voltage Reading
  ...
  if (max_v != 0) {
    VmaxD = max_v;
    VeffD = VmaxD / sqrt(2);
     Veff = (((VeffD - 557.4) / -90.24) * -220.2) + 220.2;                                                         //
  }
  if (VeffD < 543) {
    Veff = 0;
  }
